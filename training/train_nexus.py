"""넥서스 모드 멀티 정책 학습 스크립트 — 역할별 3개 PPO 모델을 라운드 로빈으로 학습"""
...
from training.callbacks import NexusBattleCallback, MultiPolicySelfPlayCallback
...
def make_env_fn(config: dict, learning_role: str):
    """역할 고정 환경 생성 팩토리."""
    def _init():
        env = NexusBattleEnv(config=config, learning_role=learning_role)
        env = Monitor(env)
        return env
    return _init
...
def train_nexus(args):
    ...
    # 정보 출력
    print("=== BattleRL Nexus Mode Multi-Policy Training ===")
    ...
    # 역할별 행동 공간 출력
    for role in roles:
        temp_env = NexusBattleEnv(config=config, learning_role=role)
    ...
    for round_idx in range(num_rounds):
        ...
        print(f"\n{'='*60}")
        print(f"  Nexus Round {round_idx + 1}/{num_rounds}")
        print(f"  Config reloaded from: {args.config}")
        print(f"{'='*60}")

        for role in roles:
            print(f"\n--- Training: {role.upper()} (Nexus Round {round_idx + 1}) ---")
        ...
        print(f"\n  Nexus Round {round_idx + 1} complete. Snapshots saved.")

    # 최종 모델 저장
    print(f"\n{'='*60}")
    print("  Nexus Training Complete!")
    print(f"{'='*60}")
    for role in roles:
...
def main():
    parser = argparse.ArgumentParser(description="BattleRL Nexus Mode Multi-Policy Training")
